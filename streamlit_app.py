...
def get_earnings_transcript(company_name, year=None, quarter=None):
    ...
    return url + "\n\n" + "Earnings Retriever Tool:\n\n"+text.split("Prepared Remarks", 1)[1].strip()[:2000]
        ...
        if st.expander("Earnings Call Transcript"):
            ...
            if st.button("Fetch Transcript", key="fetch_transcript"):
                ...
                try:
                    ...
                    analysis=rag.query(f"Get analysis for {st.session_state.company_data["name"]} with year {selected_year} and quarter {selected quarter}",lookback_hours=24)
                    st.session_state.messages.append({
                        "role": "assistant", 
